...
public class VendaMenu extends Menu{
	...
	public void acoesMenu(Integer opcao, Scanner scanner) {
		...
		switch (opcao) {
			case 1:
				try {
					System.out.println("Informe um identificador para a venda: ");
					id = scanner.nextInt();
					inserir(scanner, id);
					break;
				}
				catch(Exception e) {
					System.err.println("Identificador invalido, tente novamente.");
					break;
				}
			case 2: 
	...
	public void inserir(Scanner scanner, Integer id) {
		try {
			Venda venda = ((VendaFabrica)this.fabrica).getVenda();
			ProdutoFabrica produto = (ProdutoFabrica) Fabrica.getFabrica("produto");
			
			System.out.println("Informe abaixo as seguintes informações: Data da venda e o Cliente \n"
